...
SELECT 
    `district_id`, COUNT(`client_id`) AS `client_count`
FROM
    `client`
WHERE
    `district_id` < 10
...
SELECT 
	`type`, COUNT(`card_id`) AS `card_count`
FROM 
	`card`
GROUP BY `type`
...
SELECT 
    `account_id`, SUM(`amount`) AS `amount_sum`
FROM
    `loan`
GROUP BY `account_id`
...
SELECT 
    `date`, COUNT(`loan_id`) AS `loan_count`
FROM
    `loan`
WHERE
    `date` < 930907
